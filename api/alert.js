export default async function handler(req, res) {
  ...
  if (!apiKey) return res.status(500).json({ error: 'RESEND_API_KEY not configured in Vercel.' })
  ...
  const downside = livePrice && stopLoss ? (((stopLoss - livePrice) / livePrice) * 100).toFixed(2) : null
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Algo Trade AI <alerts@algotradeai.live>',
        to: [to],
    ...
    if (!r.ok) {
      ...
      if (msg.includes('own email') || msg.includes('testing emails')) {
        return res.status(r.status).json({ error: 'Resend free plan: you can only send to your Resend account email. Send to that email, or verify a domain at resend.com/domains.' })
      }
      return res.status(r.status).json({ error: msg || 'Resend API error' })
    }
